...
async function carregarAniversariantes() {
    try {
        const { data, error } = await db.from('pessoas')
            .select('id, nome_curto, nome_completo, foto_url, data_nascimento, celular');
    ...
    container.innerHTML = niversDaSemana.map(p => {
        ...
        return `
            <div style="display: flex; align-items: center; gap: 16px; padding: 12px; background: rgba(255,255,255,0.02); border: 1px solid var(--border); border-radius: 8px;">
                ...
                ${p.celular ? `
                <a href="https://wa.me/55${p.celular.replace(/\D/g, '')}" target="_blank" style="background: #25D366; color: white; padding: 6px 12px; border-radius: 12px; text-decoration: none; font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 6px;">
                ...
                ` : ''}
                <button onclick="abrirSideSheetResumoAniv(\'${p.id}\')"  style="cursor: pointer; border: none; background: rgba(139, 92, 246, 0.1); color: #8b5cf6; padding: 6px 12px; border-radius: 12px; font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 6px;">
