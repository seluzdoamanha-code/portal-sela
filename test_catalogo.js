...
async function run() {
    const { data: b1102 } = await supabase.from('livros_catalogo').select('*').eq('codigo', 'SELA-1102');
    console.log('SELA-1102:', b1102);
    
    const { data: b0646 } = await supabase.from('livros_catalogo').select('*').eq('codigo', 'SELA-0646');
